...
import { successResponse, errorResponse, withApiHandler } from '../../middleware'
import { createServiceLogger } from '@/server/lib/logger'

export const dynamic = 'force-dynamic'

const log = createServiceLogger('AuthMeRoute')
...
export const GET = withApiHandler(async (request: NextRequest) => {
  ...
  if (user.isDemo) {
    ...
    try {
      ...
      log.error({ err: profileErr }, 'Profile lookup failed')
    }
  }
